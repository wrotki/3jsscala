...
object Label {
  ...
  def createLabel(text: String,position: Vector3): Object3D = {
    dom.console.log("Create Label: "+text)
    ...
    val textWidth = metrics.width.toInt
    xc.fillStyle = "rgba(255,255,155,0.5)"
    xc.strokeStyle = "#000"
    xc.lineWidth = borderThickness
    xc.font = fontSize+"pt arial bold"
    roundRect(xc, borderThickness/2, borderThickness/2, textWidth + borderThickness, fontSize * 1.4 + borderThickness, 6);
    xc.fillStyle = "#000"
